...
extension BLS {
    ...
    private static func expandMessageXMDSync(
        ...
        var blocks = [Data(SHA256.hash(data: b0 + i2osp(1, 1) + dstPrime))]
        if ell > 1 {
            for i in 2...ell {
                let hashInput = Data(zip(b0, blocks[i - 2]).map { $0 ^ $1 }) + i2osp(i, 1) + dstPrime
                blocks.append(Data(SHA256.hash(data: hashInput)))
            }
        }

        return blocks.reduce(Data(), +).prefix(outputByteCount)
    }
    ...
    private static func mapToG1(_ t: Fp) throws -> SimpleProjectivePoint<Fp> {
        ...
        let denominatorInverse = try squareCandidate.inverted() * gx1
        ...
        let squareCandidateIsSquare = squareCandidate.isSquare
        let x = squareCandidateIsSquare ? x2 : x3
        let yScale = squareCandidateIsSquare ? denominatorInverse2 : alternateYScale
        let radicand = squareCandidateIsSquare ? squareCandidate : alternateSquareCandidate

        guard var y = radicand.sqrt() else {
        ...
        return try isogenyMapG1(x: x, y: y)
    }
    ...
    private static func isogenyMapG1(x: Fp, y: Fp) throws -> SimpleProjectivePoint<Fp> {
        ...
        var xnum = coefficients[index]
        index += 1
        for _ in 0..<isox {
            xnum = xnum * x + coefficients[index]
            index += 1
        }

        var xden = x + coefficients[index]
        index += 1
        for _ in 0..<(isox - 2) {
            xden = xden * x + coefficients[index]
            index += 1
        }

        var ynum = coefficients[index]
        index += 1
        for _ in 0..<isoy {
            ynum = ynum * x + coefficients[index]
            index += 1
        }

        var yden = x + coefficients[index]
        index += 1
        for _ in 0..<(isoy - 1) {
            yden = yden * x + coefficients[index]
            index += 1
        }

        ynum = ynum * y
        return SimpleProjectivePoint(
            x: xnum * yden,
            y: ynum * xden,
            z: xden * yden
        )
